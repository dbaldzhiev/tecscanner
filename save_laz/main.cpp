...
int main(int argc, char** argv)
{
...

	{
		...
		std::string dir = (slash == std::string::npos) ? std::string() : output.substr(0, slash + 1);
		std::ostringstream oss;
		oss << dir << "lidar" << std::setw(4) << std::setfill('0') << idx << ".sn";
		std::ofstream lf(oss.str());
		if(lf)
		{
			for(const auto& p : collector.serials())
			{
				lf << p.first << ' ' << p.second << '\n';
			}
		}
...

	{
		...
		std::string dir = (slash == std::string::npos) ? std::string() : output.substr(0, slash + 1);
		std::ostringstream oss;
		oss << dir << "status" << std::setw(4) << std::setfill('0') << idx << ".json";
